...
void LyraTheme::drawTabBar(const GfxRenderer& renderer, Rect rect, const std::vector<TabInfo>& tabs,
  ...
  if (totalWidth > rect.width - 2 * LyraMetrics::values.contentSidePadding) {
  ...
  for (const auto& tab : tabs) {
    ...
    renderer.drawText(UI_10_FONT_ID, currentX + padding, rect.y + 6, tab.label, !(tab.selected && selected),
