// get current path from query parameter
  ...
  async function hydrate() {
    ...
    if (files.length === 0) {
      ...
      sortedFiles.forEach(file => {
        ...
        const isFb2File = lowerFileNameForBadge.endsWith('.fb2') || lowerFileNameForBadge.endsWith('.fb2.zip');
        if (file.isDirectory) {
  ...
  function validateFile() {
    ...
    // Show convert options when at least one selected file is an EPUB, FB2, or FB2.ZIP.
    const hasConvertible = Array.from(files).some(f => {
      const n = f.name.toLowerCase();
      return n.endsWith('.epub') || n.endsWith('.fb2') || n.endsWith('.fb2.zip');
    });
...
async function convertFb2File(file, progressCallback) {
  ...
  const isZip = file.name.toLowerCase().endsWith('.fb2.zip');
  ...
  log('Cover optimization complete!', 'success', 'DONE');
...
function uploadFile() {
  ...
  const epubFilesToConvert = files.filter(f => {
    const n = f.name.toLowerCase();
    return (n.endsWith('.epub') || n.endsWith('.fb2') || n.endsWith('.fb2.zip')) && convertEnabled;
  });
  ...
  async function uploadNextFile() {
    ...
    // Check if file is an EPUB/FB2 and conversion is enabled
    const lowerFileName = file.name.toLowerCase();
    const isEpub = lowerFileName.endsWith('.epub');
    const isFb2 = lowerFileName.endsWith('.fb2') || lowerFileName.endsWith('.fb2.zip');
    const needsConversion = (isEpub || isFb2) && convertEnabled;
    ...
    try {
      ...
      if (needsConversion) {
        ...
        try {
          const converted = isFb2
            ? await convertFb2File(file, (percent) => {
                progressFill.style.width = (percent * 0.5) + '%'; // Conversion takes first 50%
              })
            : await convertEpubFile(file, (percent) => {
                // Pass current quality setting to converter
                progressFill.style.width = (percent * 0.5) + '%'; // Conversion takes first 50%
              });

          // convertFb2File resolves with a File already (possibly the original,
          // unchanged, if there was nothing to optimize); convertEpubFile
          // resolves with a Blob that needs wrapping into a File.
          file = isFb2 ? converted : new File([converted], file.name, { type: 'application/epub+zip' });
